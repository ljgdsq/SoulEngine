...
void GameApp::UpdateScene(float dt)
{
    ...
    /*
        创建3个imgui控件 可以控制water的pos
    */

    // temporary variables for ImGui sliders bound to water position
    float waterX = m_Water.GetTransform().GetPosition().x;
    float waterY = m_Water.GetTransform().GetPosition().y;
    float waterZ = m_Water.GetTransform().GetPosition().z;

    ImGui::SliderFloat("Water X", &waterX, -10.0f, 10.0f);
    ImGui::SliderFloat("Water Y", &waterY, -10.0f, 10.0f);
    ImGui::SliderFloat("Water Z", &waterZ, -10.0f, 10.0f);

    // write back slider values to the water transform
    m_Water.GetTransform().SetPosition(waterX, waterY, waterZ);
